# Redis Connect for Oracle in K8s

This repository describes the steps involved to deploy Redis Connect for Oracle in K8s. 

Overall flow:
1. Clone the Redis Connect for Oracle repository.
2. Configure Redis Connect as in <a href="../demo/config/samples/oracle" target="_blank">this set of docs</a>.
...
Configure the files to describe your Redis Connect Job. One sample configuration is <a href="../demo/config/samples/oracle" target="_blank">here</a>. 
...
* Source database details (endpoint, port, credentials)
  * <a href="../" target="_blank">LogMiner and replication configuration</a> completed on the source database system
* Source schema details
* Target Redis details and instances (one for the data, one for the Job configuration)

Details for configuring Redis Connect for Oracle are <a href="../demo/" target="_blank">here</a>.
...
```
kubectl create configmap redis-connect-oracle-config \
...
The outcome is:
```
$ oc get configmap/redis-connect-oracle-config
NAME                            DATA   AGE
redis-connect-oracle-config   5      5s
```
...
$ oc get configmap/redis-connect-oracle-config
NAME                            DATA   AGE
redis-connect-oracle-config   6      12s
```
...
```
        volumeMounts:
        - name: config-volume
          mountPath: /opt/redislabs/redis-connect-oracle/config/fromconfigmap
```
The volume is defined through the following `volume` directive. It will mount the file/pth `JobConfig.yml` using the contents of the key named `JobConfig.yml` from the ConfigMap `redis-connect-oracle-config` in the `mountPath` define above.  
```
      volumes:
      - name: config-volume
        configMap:
...
```
root@redis-connect-oracle-7b7ccf87b9-sqshl> pwd
/opt/redislabs/redis-connect-oracle/config/fromconfigmap
root@redis-connect-oracle-7b7ccf87b9-sqshl> ls -al
total 0
...
Update both the `redis-connect-oracle-stage.yaml` and `redis-connect-oracle-start.yaml` to map the appropriate environment variables in the `env:` section. Notable, the `REDISCONNECT_SOURCE_USERNAME`, `REDISCONNECT_SOURCE_PASSWORD`, `REDISCONNECT_TARGET_USERNAME` and `REDISCONNECT_TARGET_PASSWORD`.  
...
Apply the stage manifest as follows: `oc apply -f redis-connect-oracle-stage.yaml`. The outcome will be a k8s batch/Job which will run once and exit. 
```
$ oc get po -w 
NAME                                         READY   STATUS      RESTARTS   AGE
redis-connect-oracle-stage-lkvp2           0/1     Completed   0          44s
``` 
...
Apply the stage manifest as follows: `oc apply -f redis-connect-oracle-start.yaml`. The outcome will be a k8s apps/Deployment which will run continually. 
```
$ oc get po -w
NAME                                         READY   STATUS      RESTARTS   AGE
redis-connect-oracle-cbc7dcd9d-k9mxj       1/1     Running     0          4s
redis-connect-oracle-stage-lkvp2           0/1     Completed   0          3m10s
``` 

The effect of the above is that the Redis Connect job has started. The Job Owner should be specified in the in `env.yml`:`jobConfigConnection` Redis DB as `JC-xx@redis-connect-oracle-cbc7dcd9d-k9mxj` indicating that the pod created is the Redis Connect job owner. You should see your changes propagate to the `targetConnection` Redis database as defined in `env.yml`. 

---
### Troubleshooting Options

1. Tail the pod logs. `oc logs -f pod/redis-connect-oracle-cbc7dcd9d-k9mxj`
