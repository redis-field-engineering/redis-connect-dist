...
curl -v -k -L -u demo@redislabs.com:redislabs --location-trusted -H Content-type:application/json -d '{ "name": "RedisCDC-Target-db", "port": 12000, "memory_size": 100000000, "type" : "redis", "replication": false, "module_list": [{"module_args": "PARTITIONS AUTO", "module_id": "f181a538611833224950c3d157bd89f9", "module_name": "search", "semantic_version": "2.2.0"}]}' https://localhost:9443/v1/bdbs
curl -v -k -L -u demo@redislabs.com:redislabs --location-trusted -H "Content-type:application/json" -d '{"name": "RedisCDC-JobConfig-Metrics-db", "type":"redis", "replication": false, "memory_size":100000000, "port":12001, "module_list": [{"module_args": "", "module_id": "f3b681f2c740cf9af3bffd5eef302166", "module_name": "timeseries", "semantic_version": "1.2.7"}]}' https://localhost:9443/v1/bdbs
EOF
...
#sleep 60
# Create RedisSearch index with matching schema as the emp hash to be able to search/aggregate etc.
sudo docker exec -it re-node1 bash -c "/opt/redislabs/bin/redis-cli -p 12000 ft.create emp on hash prefix 1 emp: schema EmpNum numeric sortable FName text sortable LName text Job tag sortable Manager numeric HireDate text Salary numeric Commission numeric Department numeric"
...
sudo docker exec -it re-node1 bash -c "rladmin status nodes && rladmin status databases && rladmin status shards"
